...
export class AppComponent implements OnDestroy{
  ...
  ngOnInit(): void {
    ...
    this.userSub = this.authService.user.subscribe(user=>{
      ...
      this.workspaceService.url.searchParams.append('topic', 'https://todolist.com/workspaces/'+this.currentUser?.id);
      this.workspaceService.url.searchParams.append('topic', 'https://todolist.com/lists/workspaces/'+this.currentUser?.id);
      this.workspaceService.url.searchParams.append('topic', 'https://todolist.com/tasks/workspaces/'+this.currentUser?.id);
      this.eventSource = new EventSource(this.workspaceService.url);
      this.eventSource.onmessage = ev => this.handleSynchronizedData(JSON.parse(ev.data));
    });
  }
  ...
  handleSynchronizedData(data: any) {
    //workspaces sync
    if (data.class == 'workspace'){
      if (data.type == 'create') {
        ...
        if (!!data.name){
          ...
          if (data.innerType == 'addLists' && data.ids.length > 0) {
            ...
            this.workspaceService.removeSharedLists(data.ids,data.id);
          }else if (data.innerType == 'addUsers') {
            let workspace = new Workspace(data.originName,data.owner,data.names);
            workspace.id = data.id;
            workspace.sharedLists = newLists;
            workspace.sharedSideLists = sideLists;
            workspace.sharedUsers = data.names;
            this.workspaceService.addWorkspace(workspace);
    ...
    //lists sync
      ...
      if (data.type == 'create'){
        ...
        data.ids.forEach( (id,index) => {
          this.workspaceService.removeSharedList(id,data.id);
    ...
    //tasks sync
      ...
      if (data.type == 'create'){
        let newTask = new Task(
          ...
          data.endTime,
          false);
        this.taskService.isAddTask.next([newTask,data.listId]);
      }else if (data.type == 'delete'){
        data.ids.forEach( id => {
